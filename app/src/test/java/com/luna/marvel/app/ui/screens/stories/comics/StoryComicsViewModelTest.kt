...
class StoryComicsViewModelTest {
    ...
    @Mock
    lateinit var getComicsByStoryIdUseCase: GetComicsByStoryIdUseCase

    private val state = StoryComicsViewModel.State()
    ...
    fun `on ViewModel initialization downloads a list of comics from service`() = runTest {
        ...
        whenever(getComicsByStoryIdUseCase(1)).thenReturn(Either.Right(comics))
        val viewModel = StoryComicsViewModel(savedStateHandle, getComicsByStoryIdUseCase)
    ...
    fun `on ViewModel initialization downloads an empty list of comics from service and sets AppError on state`() =
        runTest {
            ...
            val viewModel =
                StoryComicsViewModel(savedStateHandle, getComicsByStoryIdUseCase)
