...
class CharacterSeriesViewModelTest {
    ...
    @Mock
    lateinit var getCharacterSeriesByIdUseCase: GetCharacterSeriesByIdUseCase

    private val state = CharacterSeriesViewModel.State()
    ...
    fun `on ViewModel initialization downloads a series list from service`() = runTest {
        ...
        whenever(getCharacterSeriesByIdUseCase(1)).thenReturn(Either.Right(series))
        val viewModel = CharacterSeriesViewModel(savedStateHandle, getCharacterSeriesByIdUseCase)
    ...
    fun `on ViewModel initialization downloads an empty list of series from service and sets AppError on state`() =
        runTest {
            ...
            val viewModel =
                CharacterSeriesViewModel(savedStateHandle, getCharacterSeriesByIdUseCase)
