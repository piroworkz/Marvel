...
class EventComicsViewModelTest {
    ...
    @Mock
    lateinit var getComicsByEventIdUseCase: GetComicsByEventIdUseCase
    
    private val state = EventComicsViewModel.State()
    ...
    fun `on ViewModel initialization downloads a list of comics from service`() = runTest {
        ...
        whenever(getComicsByEventIdUseCase(1)).thenReturn(Either.Right(comics))
        val viewModel = EventComicsViewModel(savedStateHandle, getComicsByEventIdUseCase)
    ...
    fun `on ViewModel initialization downloads an empty list of comics from service and sets AppError on state`() =
        runTest {
            ...
            val viewModel =
                EventComicsViewModel(savedStateHandle, getComicsByEventIdUseCase)
