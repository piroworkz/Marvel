package com.luna.marvel.app.ui.screens.events.series
...
class EventSeriesViewModelTest {
    ...
    @Mock
    lateinit var getSeriesByEventIdUseCase: GetSeriesByEventIdUseCase

    private val state = EventSeriesViewModel.State()
    ...
    fun `on ViewModel initialization downloads a series list from service`() = runTest {
        ...
        whenever(getSeriesByEventIdUseCase(1)).thenReturn(Either.Right(series))
        val viewModel = EventSeriesViewModel(savedStateHandle, getSeriesByEventIdUseCase)
    ...
    fun `on ViewModel initialization downloads an empty list of series from service and sets AppError on state`() =
        runTest {
            ...
            val viewModel =
                EventSeriesViewModel(savedStateHandle, getSeriesByEventIdUseCase)
