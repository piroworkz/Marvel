...
class ComicCreatorsViewModelTest {
    ...
    @Mock
    lateinit var getComicCreatorsByIdUseCase: GetComicCreatorsByIdUseCase

    private val state = ComicCreatorsViewModel.State()
    ...
    fun `on ViewModel initialization downloads a list of creators from service`() = runTest {
        ...
        whenever(getComicCreatorsByIdUseCase(1)).thenReturn(Either.Right(creators))
        val viewModel = ComicCreatorsViewModel(savedStateHandle, getComicCreatorsByIdUseCase)
    ...
    fun `on ViewModel initialization downloads an empty list of creators from service and sets AppError on state`() =
        runTest {
            ...
            val viewModel =
                ComicCreatorsViewModel(savedStateHandle, getComicCreatorsByIdUseCase)
